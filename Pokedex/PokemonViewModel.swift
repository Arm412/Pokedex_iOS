...
class PokemonViewModel: ObservableObject {
    ...
    init(controller: FetchController) {
        ...
        // Need task to run this function in init
        Task {
            await getPokemon()
        }
    }
    ...
    private func getPokemon() async {
        ...
        do {
            guard var pokedex = try await controller.fetchAllPokemon() else {
                print("Pokemon are already fetched.")
                status = .success
                return
            }
            ...
            for pokemon in pokedex {
                ...
                newPokemon.organizeTypes() // If applicable, swap the normal type with the pokemon's second type
